...
Partial Class GF_ctUserProject
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/CT_Main/App_Display/DF_ctUserProject.aspx"
  ...
  End Sub
  Protected Sub GVctUserProject_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GVctUserProject.RowCommand
    If e.CommandName.ToLower = "lgedit".ToLower Then
      Try
        Dim UserID As String = GVctUserProject.DataKeys(e.CommandArgument).Values("UserID")  
        Dim ProjectID As String = GVctUserProject.DataKeys(e.CommandArgument).Values("ProjectID")  
        Dim RedirectUrl As String = TBLctUserProject.EditUrl & "?UserID=" & UserID & "&ProjectID=" & ProjectID
        Response.Redirect(RedirectUrl)
    ...
    If e.CommandName.ToLower = "Deletewf".ToLower Then
      Try
        Dim UserID As String = GVctUserProject.DataKeys(e.CommandArgument).Values("UserID")  
        Dim ProjectID As String = GVctUserProject.DataKeys(e.CommandArgument).Values("ProjectID")  
        SIS.CT.ctUserProject.DeleteWF(UserID, ProjectID)
        GVctUserProject.DataBind()
      Catch ex As Exception
        ScriptManager.RegisterClientScriptBlock(Page, Page.GetType(), "", "alert('" & New JavaScriptSerializer().Serialize(ex.Message) & "');", True)
      End Try
    End If
  End Sub
  Protected Sub GVctUserProject_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVctUserProject.Init
    DataClassName = "GctUserProject"
    SetGridView = GVctUserProject
  End Sub
  ...
  Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
    ...
    Dim validateScriptUserID As String = "<script type=""text/javascript"">" & _
      ...
      "    validated_FK_CT_UserProject_UserID_main = true;" & _
      "    validate_FK_CT_UserProject_UserID(o);" & _
      "  }" & _
      "</script>"
    ...
    Dim validateScriptProjectID As String = "<script type=""text/javascript"">" & _
      ...
      "    validated_FK_CT_UserProject_ProjectID_main = true;" & _
      "    validate_FK_CT_UserProject_ProjectID(o);" & _
      "  }" & _
      "</script>"
    ...
    End If
    Dim validateScriptFK_CT_UserProject_UserID As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_CT_UserProject_UserID(o) {" & _
      "    var value = o.id;" & _
      "    var UserID = $get('" & F_UserID.ClientID & "');" & _
      "    try{" & _
      "    if(UserID.value==''){" & _
      "      if(validated_FK_CT_UserProject_UserID.main){" & _
      ...
      "    PageMethods.validate_FK_CT_UserProject_UserID(value, validated_FK_CT_UserProject_UserID);" & _
      "  }" & _
      "  validated_FK_CT_UserProject_UserID_main = false;" & _
      "  function validated_FK_CT_UserProject_UserID(result) {" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_CT_UserProject_UserID") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_CT_UserProject_UserID", validateScriptFK_CT_UserProject_UserID)
    End If
    Dim validateScriptFK_CT_UserProject_ProjectID As String = "<script type=""text/javascript"">" & _
      "  function validate_FK_CT_UserProject_ProjectID(o) {" & _
      "    var value = o.id;" & _
      "    var ProjectID = $get('" & F_ProjectID.ClientID & "');" & _
      "    try{" & _
      "    if(ProjectID.value==''){" & _
      "      if(validated_FK_CT_UserProject_ProjectID.main){" & _
      "        var o_d = $get(o.id +'_Display');" & _
      "        try{o_d.innerHTML = '';}catch(ex){}" & _
      "      }" & _
      "    }" & _
      ...
      "    o.style.backgroundRepeat= 'no-repeat';" & _
      "    o.style.backgroundPosition = 'right';" & _
      "    PageMethods.validate_FK_CT_UserProject_ProjectID(value, validated_FK_CT_UserProject_ProjectID);" & _
      "  }" & _
      "  validated_FK_CT_UserProject_ProjectID_main = false;" & _
      "  function validated_FK_CT_UserProject_ProjectID(result) {" & _
      "    var p = result.split('|');" & _
      "    var o = $get(p[1]);" & _
      "    var o_d = $get(p[1]+'_Display');" & _
      "    try{o_d.innerHTML = p[2];}catch(ex){}" & _
      ...
      "  }" & _
      "</script>"
    If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_CT_UserProject_ProjectID") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_CT_UserProject_ProjectID", validateScriptFK_CT_UserProject_ProjectID)
    End If
  End Sub
  <System.Web.Services.WebMethod()> _
